...
최종적으로 아래 UI를 구현해야한다. 실질적으로 유저에게 나오는 화면을 로그로 찍어서 문제 없이 아래의 화면이 나올 때 까지 빌드 & 강제 종료 및 재실행 & 로그 확인 & 크래시 안나는지도 확인 (앱 켠 후 30초간 모니터링)을 반복해. 나에게 확인해달라고 요구하지 말고.

프로바이더는 OpenRouter, OpenCode Zen, Copilot Add-on, Claude, Codex, Gemini CLI, Antigravity, Copilot 이 인식되어야하고, Opencode Zen, Copilot Add-on 은 usage history 가 일별로 나와야한다.

완료할 때 까지 반복하며, 한개 파일의 소스코드가 너무 길어지면 (700라인 이상) 파일을 매우 직관적인 파일명(클래스명)으로 적절히 쪼개라. 클래스명이나 파일명이 매우 길어져도 괜찮다.
